...
class CustomSugarFieldEnum extends SugarFieldBase {
    ...
    /*
    * Custom display field enum on listview
    */
    function getListViewSmarty($parentFieldArray, $vardef, $displayParams, $tabindex) {
        if(isset($vardef['name']) && $vardef['name'] == 'PARENT_TYPE'){
            global $app_list_strings;
            $str = '';
            if(!empty($app_list_strings[$vardef['options']]) && !empty($vardef['name'])) {
                $str = $app_list_strings[$vardef['options']][$parentFieldArray[$vardef['name']]];
            }
            return $str;
        }
        else {
