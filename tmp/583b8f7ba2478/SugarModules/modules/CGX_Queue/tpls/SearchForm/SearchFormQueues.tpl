...
{{/foreach}}
		...
		<td scope="row" nowrap="nowrap" width="1%">
			<label for="current_user_only_basic">Business Process</label>
		</td>
		<td nowrap="nowrap" width="1%">
			<select name="business_process_name[]" size="6" style="width: 150px" multiple="1">
			{{foreach name=business from=$businessProcessNames key=col item=colData}}
				<option label="{{$colData}}" value="{{$colData}}">{{$colData}}</option>
			{{/foreach}}
			</select>
		</td>
