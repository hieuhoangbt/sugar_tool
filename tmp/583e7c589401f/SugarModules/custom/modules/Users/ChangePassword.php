    ...
    try {
        ...
        if (!empty($response['users']['id'])) {
            ...
            if (empty($response['statusCode']) || $response['statusCode'] != '000') {
                $GLOBALS['log']->debug('ChangePassword::' . print_r($post_response, true));
            }
            ...
            updateEncryptedPassword($focus->id, $cgx_encrypted_password_c, $cgx_pad_password_c);
        } else {
            if (!empty($response['status']['statusCode']) && $response['status']['statusCode'] = '001') {
                $GLOBALS['log']->debug('ChangePassword::getCasegenixId::userNotExists::' . $userName);
            }
            $GLOBALS['log']->debug('ChangePassword::' . print_r($userExists, true));
        }
    } catch (Exception $e) {
        $GLOBALS['log']->debug('ChangePassword::' . $e->getMessage());
    }
//#hdwebsoft customise
