<?php
    ...
    ********************************************************************************/
            ...
            if (!$expired)
            {
                ...
                if (isset($_REQUEST['login']) && $_REQUEST['login'] =='1'){
                    if ( $row['username'] == $_POST['user_name'] ){
                        ...
                        //hdwebsoft customise    
                        //Begin SE-364
                        if(file_exists('modules/CGX_API/api/CGX_Kit_Factory.php')) {
                            //End SE-364
                            try{
                                ...
                                if(!empty($response['users']['id'])){
                                    ...
                                    if(empty($response['statusCode']) || $response['statusCode'] != '000'){
                                        $GLOBALS['log']->debug('Changenewpassword::'.print_r($post_response, true));
                                    }

                                    // update encrypted password
                                    require_once('custom/modules/Users/utils/helpers.php');
                                    if($_POST['record'] == "1"){
                                        die();
                                    }
                                    $kitInOut = $cgx_kit->getInOut();
                                    $cgx_encrypted_password_c = $kitInOut->encryptPassword($password);
                                    $cgx_pad_password_c = $kitInOut->getPad();
                                    updateEncryptedPassword($usr_id, $cgx_encrypted_password_c, $cgx_pad_password_c);
                                }else{
                                    if(!empty($response['status']['statusCode']) && $response['status']['statusCode'] = '001'){
                                        $GLOBALS['log']->debug('Changenewpassword::getCasegenixId::userNotExists::'.$userName);
                                    }
                                    $GLOBALS['log']->debug('Changenewpassword::'.print_r($userExists, true));
                                }
                            }catch(Exception $e){
                                $GLOBALS['log']->debug('Changenewpassword::'.$e->getMessage());
                            }
                            //Begin SE-364
                        }
                        //End SE-364
    ...
    $sugar_smarty->assign('sugar_md',$login_image_url);
    //End SE-3644
